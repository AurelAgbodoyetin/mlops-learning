...
def print_metrics(model_name, alpha, l1_ratio, rmse, mae, r2): # <- Updated - Modified to include model_name and handle None
    print("\n" + "="*50)
    print(f"{model_name.upper()} MODEL PERFORMANCE")
    print("="*50)
    
    # Handle None for alpha and l1_ratio
    alpha_display = f"{alpha:.4f}" if alpha is not None else "NaN"
    l1_ratio_display = f"{l1_ratio:.4f}" if l1_ratio is not None else "NaN"
    
    print(f"Parameters: alpha = {alpha_display}, l1_ratio = {l1_ratio_display}")
    print("-"*50)
...
if __name__ == "__main__":
    ...
    ##### FIRST EXPERIMENT ######
    # Using elasticnet regression to predict wine quality

    # Create a new experiment
    experiment_obj = mlflow.set_experiment(experiment_name="Exp-Multi-1-ElasticNet-Wine-Quality")
    ...
    # Print the results
    print_metrics("ElasticNet", alpha, l1_ratio, rmse, mae, r2)
    ...
    ##### SECOND EXPERIMENT ######
    # Using ridge regression to predict wine quality

    # Create a new experiment
    experiment_obj = mlflow.set_experiment(experiment_name="Exp-Multi-1-Ridge-Wine-Quality")
    experiment_id = experiment_obj.experiment_id
    ...
    # Set the parameters
    alpha = 0.6 # <- Changed value
    l1_ratio = None  # Not used in Ridge
    ...
    # Create and train the Ridge model with specified parameters
    ...
    # Print the results
    print_metrics("Ridge", alpha, None, rmse, mae, r2)
    ...
    # Log the trained model to MLflow
    mlflow.sklearn.log_model(lr, name="ridge-model", input_example=X_test.iloc[0:])
    mlflow.log_artifact("./launch-multiple-runs.py")  # Log this script as an artifact
    mlflow.log_artifacts("data")  # Log the data directory as artifacts
    ...
    ##### THIRD EXPERIMENT ######
    # Using lasso regression to predict wine quality

    # Create a new experiment
    experiment_obj = mlflow.set_experiment(experiment_name="Exp-Multi-1-Lasso-Wine-Quality")
    experiment_id = experiment_obj.experiment_id
    ...
    # Set the parameters
    alpha = 0.6
    l1_ratio = None  # Not used in Lasso
    ...
    # Create and train the Lasso model with specified parameters
    ...
    # Print the results
    print_metrics("Lasso", alpha, None, rmse, mae, r2)
    ...
    # Log the trained model to MLflow
    mlflow.sklearn.log_model(lr, name="lasso-model", input_example=X_test.iloc[0:])
    mlflow.log_artifact("./launch-multiple-runs.py")  # Log this script as an artifact
    mlflow.log_artifacts("data")  # Log the data directory as artifacts
