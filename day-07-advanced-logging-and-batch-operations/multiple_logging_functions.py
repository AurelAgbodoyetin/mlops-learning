...
if __name__ == "__main__":
    ...
    if not os.path.exists("data"):  # <- New line
        os.mkdir("data") # <- New line
    ...
    data.to_csv("data/winequality.csv", index=False) # <- New line
    ...
    X_train.to_csv("data/X_train.csv", index=False) # <- New line
    X_test.to_csv("data/X_test.csv", index=False)   # <- New line
    y_train.to_csv("data/y_train.csv", index=False) # <- New line
    y_test.to_csv("data/y_test.csv", index=False)   # <- New line
    ...
    mlflow.start_run()  
    mlflow.set_tag("release.version", "0.1")  # <- New line
    tags = { # <- New line
    ...
    mlflow.log_params(params) # <- New line
    ...
    mlflow.log_metrics(metrics) # <- New line
