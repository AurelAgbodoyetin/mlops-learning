...
# New function to print active and last active run IDs
def print_active_and_last_active_run_ids(): # New function
    active_run = mlflow.active_run()
    if active_run is None:
        print("No active run")
        return
    print("Active run id is {}".format(active_run.info.run_id))
    last_active_run = mlflow.last_active_run()
    print("Last active run id is {}".format(last_active_run.info.run_id))
...
if __name__ == "__main__":
    ...
    with mlflow.start_run(experiment_id=experiment_id, run_name="ElasticNet-Run-1"):
        ...
        # Log the active and last active run IDs inside the with block
        print_active_and_last_active_run_ids()

    # The run is automatically ended when exiting the 'with' block

    # Log the active and last active run IDs outside the with block
    print_active_and_last_active_run_ids()
    ...
    # Log the active and last active run IDs in between start_run() and end_run()
    print_active_and_last_active_run_ids()

    mlflow.end_run()  

    # Log the active and last active run IDs after end_run()
    print_active_and_last_active_run_ids()